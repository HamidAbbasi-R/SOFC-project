def save_data(data_name, phi, residuals):
    ...
    with open('Binary files/case_data/' + data_name + '.pkl', 'wb') as file:
        dump(save_obj, file)
...
def load_case_individual(case_name):
    ...
    with warnings.catch_warnings(): 
        ...
        with open('Binary files/case_data/' + case_name + '.pkl', 'rb') as file:
    ...
    for p in [0,1,2]:
        if inputs['solver_options']['ion_only'] and p!=2:
            continue
        J[p] = load_npz('Binary files/case_data/' + case_name + f'_sparse_{p}.npz').tolil()
...
def save_case_individual(case_name, 
    ...
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        with open('Binary files/case_data/' + case_name + '.pkl', 'wb') as case_file:
            dump(save_obj, case_file)
    
    for p in [0,1,2]:
        if inputs['solver_options']['ion_only'] and p!=2:
            continue
        save_npz('Binary files/case_data/' + case_name + f'_sparse_{p}.npz', J[p].tocsr())
...
def load_case_data_individual(case_name, data_name):
    ...
    with open('Binary files/case_data/' + data_name + '.pkl', 'rb') as data_file:
